...
. For each animation, check its *Loop Time* checkbox in the Inspector - this causes the animation to loop indefinitely when it is active.
...
image::walk.gif[]
+
Depending on how the scene is set up, you may also see it in the *Game* window, but since we haven't implemented any game-time controls (e.g. zoom or rotate etc.) or made any camera or lighting adjustments, it may display a less-than-perfect framing of our character.
