...
. Once again, expand the *ybot@Walking* asset in our *Assets* > *Mixamo* window, and duplicate its embedded animation by selecting it and pressing CTRL-D. This action creates an independent animation that's separate from its parent model.
+
...
+
Depending on how the scene is set up, you may also see it in the *Game* window, but since we haven't enabled any game-time controls (e.g. zoom or rotate) it may display a less-than-perfect view of our character.

. To view the other animations, drag them into the *Animator* window, right-click and choose *Set as Layer Default State* and then press *Play*. Here, I've set the *Running* animation as default.
+
