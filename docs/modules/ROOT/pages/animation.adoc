...
. Drag an animation from the *Mixamo* > *Assets* folder into the scene and reset its transform (right-click on its *Transform* component header in the Inspector and choose *Reset*). I've chosen the _walking_ animation but _running_ would be just as suitable. The movements in the _idle_ animation are very subtle and would probably hinder this first walkthrough. 
+
The scene window shows the model, now located at the world origin, and the Hierarchy pane now contains a *ybot@Walking* entry which you can expand to review its component parts.
+
[#img-ybot-in-hierarchy]
.ybot in the Hierarchy view
image::ybot-in-hierarchy.png[]
+
You can see how it is composed of two meshes (each of which has its own material) and a rig, which further decomposes into its own subcomponents.